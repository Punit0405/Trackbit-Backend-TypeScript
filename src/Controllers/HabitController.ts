...
class HabitClass {
  ...
  public deleteHabit = async (req: RequestUser, res: Response) => {
    ...
    if (habit.type) {
      return res
        .status(400)
        .json({ status: false, data: "You cannot delete challange habit" });
    }
    if (habit.userId.toString() !== req.user.id) {
      return res.status(400).json({
        status: false,
  ...
  public habitNotification = async () => {
    try {
      const habits = await Habit.find({ type: false }).populate({
        path: "userId",
      });
      const challangeHabit = await Habit.find({ type: true }).populate({
        path: "challangeId",
        populate: {
          path: "participants",
        },
      ...
      if (
        notificationHabits.length === 0 &&
        notifiChallangeHabits.length === 0
      ) {
        return 0;
      }
      ...
      notifiChallangeHabits.forEach((habit) => {
        ...
        habit.challangeId.participants.forEach((participant: any) => {
          ...
          Notifier.sendNotification(
            ...
            "Its Time To Follow Your challange Habit",
            habit.title
          );
        });
      });
