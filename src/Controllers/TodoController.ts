...
class TodoClass {
  ...
  public updateTodo = async (req: RequestUser, res: Response) => {
    ...
    if (todo.userId.toString() !== req.user.id) {
      return res
        .status(400)
        .json({ status: false, data: "Todo doesn't Exists for this Account" });
    }
  ...
  public todoNotification = async () => {
    try {
      const todos = await Todo.find({ type: false }).populate({
        path: "userId",
      });
      const challaneTodo = await Todo.find({ type: true }).populate({
        path: "challangeId",
        populate: {
          path: "participants",
        },
      ...
      todos.forEach((todo: any) => {
        if (
          ...
          todo.reminderDate.getMonth() === month &&
          todo.reminderDate.getFullYear() === year
        ) {
