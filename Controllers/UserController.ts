...
class UserClass {
  ...
  public verifyUser = async (req: RequestUser, res: Response) => {
    ...
    try {
      ...
      if (!databaseCheck) {
        return res
          .status(400)
          .json({ status: false, data: "User Not Registered,Please Register" });
      }
  ...
  public sendVerifyLink = async (req: RequestUser, res: Response) => {
    ...
    if (!email) {
      return res
        .status(404)
        .json({ status: false, data: "Please Provide Email Iconsole.log" });
    }
    const user = await User.findOne({ email: email });
    if (!user) {
      return res
        .status(404)
        .json({ status: false, data: "User not exists,Please Register first" });
    }
    if (user.email_verified) {
      return res
        .status(400)
        .json({ status: false, data: "Email is already verified" });
    }
    try {
      const token = await jwt.sign(
        { email: email },
    ...
    if (await bcrypt.compare(userPassword, user.password)) {
      if (!user.email_verified) {
        return res
          ...
          .json({
            status: false,
            data: "Email is not verified, Please Verify",
          });
      }
      const loginData = {
        id: user._id,
  ...
  public userGoogleLogin = async (req: RequestUser, res: Response) => {
    ...
    if (!payload) {
      return res.status(400).json({ status: false, data: "Token is invalid" });
    }
